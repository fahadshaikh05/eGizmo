...
<sql:setDataSource var="myDS" driver="com.mysql.jdbc.Driver"
        ...
        select p.productId,p.name,p.cost,p.imagePath from shoppingcart c   join  product p on c.productId=p.productId where buyerId=<%=(Integer)session.getAttribute("buyerId")%>; 
    </sql:query>
<table id= "phones"   cellpadding="5" >
				<c:forEach var="prod" items="${wishlist.rows}">
					<tr>
						...
						<td><form action="checkout.jsp?productId=<c:out value="${prod.productId}"/>" method="post"><input type="submit" value="Proceed Checkout"></form>
						<form action="removeFromShoppingCart.jsp?productId=<c:out value="${prod.productId}"/>" method="post"><input type="submit" value="Remove from Cart"></form></td>				
					</tr>
				</c:forEach>   
</table>
</div>
