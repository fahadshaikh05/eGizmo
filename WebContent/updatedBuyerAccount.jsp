...
<%
	...
	<div id="register">
		...
		<sql:update dataSource="${dbsource}" var="count">
            UPDATE buyer SET  firstName=?,lastName=?,middleName=?,emailAddress=?,
             streetAddress=?, city=?, state=? ,pincode=?,phoneNumber=? 
            WHERE buyerId=<%=(Integer)session.getAttribute("buyerId")%>
            <sql:param value="${param.firstName}" />
			<sql:param value="${param.lastName}" />
			<sql:param value="${param.middleName}" />
			<sql:param value="${param.emailAddress}" />
			<sql:param value="${param.streetAddress}" />
