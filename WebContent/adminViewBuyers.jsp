	...
	<div id="register">
		<form id="customers">
			...
			<sql:query var="listProducts" dataSource="${myDS}">
				SELECT * FROM buyer where buyerId NOT IN (123);
    </sql:query>

			<div align="center">
				<table border="1" cellpadding="5">
					<caption>
						<h2>List of Buyers's</h2>
					</caption>
					<tr>
					...
					<c:forEach var="prod" items="${listProducts.rows}">
						<tr>
							...
							<td><c:out value="${prod.middleName}" /></td>
							<td><c:out value="${prod.lastNamee}" /></td>
							<td><c:out value="${prod.emailAddress}" /></td>
							<td><c:out value="${prod.phoneNumber}" /></td>
							<td><c:out value="${prod.streetAddress}" /></td>
							<td><c:out value="${prod.city}" /></td>
							<td><c:out value="${prod.state}" /></td>
							<td><c:out value="${prod.pincode}" /></td>
							<td><c:out value="${prod.userName}" /></td>
							<td><c:out value="${prod.password}" /></td>
							<td><c:out value="${prod.isActive}" /></td>
							<td><a
								href="adminDeleteBuyer.jsp?buyerId=<c:out value="${prod.buyerId}"/>">Block</a></td>
							<td><a
								href="adminActivateBuyer.jsp?buyerId=<c:out value="${prod.buyerId}"/>">Activate</a></td>
